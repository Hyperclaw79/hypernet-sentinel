...
use measurement_core::{
    DiagnosticKind, DiagnosticOutcome, MeasurementReport, TestSelection, UPSTREAM_REVISION,
};
...
impl Database {
    pub async fn open(data_dir: &Path) -> Result<Self> {
        ...
        db.call(|connection| {
            ...
            if version < 4 {
                connection
                    .execute_batch(include_str!("../migrations/004_diagnostic_outcomes.sql"))?;
            }
            Ok(())
        })
        .await?;
        ...
        self.call(move |connection| {
            connection.execute("UPDATE test_runs SET status='completed',completed_at=?2,measurement_target=?3,latency_ms=?4,jitter_ms=?5,packet_loss_pct=?6,download_mbps=?7,upload_mbps=?8,raw_result_json=?9,loaded_latency_download_ms=?10,loaded_latency_upload_ms=?11,download_bufferbloat_ms=?12,upload_bufferbloat_ms=?13,selected_tests_json=?14,outcome=?15,outcome_detail=?16,error=NULL WHERE id=?1",
                params![id, completed, report.measurement_target, report.latency_ms, report.jitter_ms, report.packet_loss_pct,
                    ...
                    serde_json::to_string(&report.selected_tests)?, outcome_text(report.outcome), report.outcome_detail])?;
            Ok(())
        }).await
    }
    ...
    pub async fn is_due(&self, kind: DiagnosticKind, interval: Duration) -> Result<bool> {
        self.call(move |connection| {
            let last: Option<i64> = connection.query_row("SELECT MAX(unixepoch(started_at)) FROM test_runs WHERE test_kind=?1 AND status IN ('running','completed','failed')", [kind_text(kind)], |r| r.get(0))?;
...
fn outcome_text(outcome: DiagnosticOutcome) -> &'static str {
    match outcome {
        DiagnosticOutcome::Measured => "measured",
...
mod tests {
    ...
    async fn migrations_and_measurement_round_trip_preserve_nulls_and_raw_data() {
        ...
        db.complete(
            ...
            MeasurementReport {
                ...
                outcome: measurement_core::DiagnosticOutcome::Measured,
                outcome_detail: None,
    ...
    #[tokio::test]
    async fn migration_reclassifies_legacy_no_sample_failures_as_unavailable() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("sentinel.db");
        let connection = Connection::open(&path).unwrap();
        connection
        ...
        connection
            ...
            .unwrap();
        let error = "quality measurement returned no valid samples: HTTP latency 0/1 probes";
        connection.execute(
            "INSERT INTO test_runs (id,test_kind,trigger,status,started_at,completed_at,error,upstream_engine_revision,selected_tests_json) VALUES ('legacy-outage','quality','scheduled','failed',strftime('%Y-%m-%dT%H:%M:%fZ','now'),strftime('%Y-%m-%dT%H:%M:%fZ','now'),?1,'revision','{\"latency\":true,\"download\":false,\"upload\":false,\"packet_loss\":true}')",
            [error],
        ).unwrap();
        drop(connection);
        ...
        assert_eq!(run.outcome, Some(DiagnosticOutcome::Unavailable));
        assert_eq!(run.outcome_detail.as_deref(), Some(error));
        assert!(run.error.is_none());
        assert!(run.latency_ms.is_none());
        assert!(run.packet_loss_pct.is_none());
    }

    #[tokio::test]
    async fn failed_attempt_advances_the_scheduler_due_clock() {
        let directory = tempdir().unwrap();
        let db = Database::open(directory.path()).await.unwrap();
        db.insert_running(
            "failed-attempt".into(),
        ...
        assert!(!db
            .is_due(DiagnosticKind::Quality, Duration::from_secs(3600))
            .await
            .unwrap());
    }
}
